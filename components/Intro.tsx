    ...
    <section ref={ref} id='home' className='scroll-mt-[100rem] mb-28 sm:mb-0 max-w-[50rem] text-center'>
      <div className='flex justify-center items-center'>
        <div className='relative'>
          ...
          >
            <Image src={profileImage} alt='Akshay Gupta' width="192" height="192" quality="95" priority={true} className='border-[0.35rem] border-white shadow-xl rounded-full w-24 h-24 object-cover' />
          </motion.div>
          <motion.span
            initial={{ opacity: 0, scale: 0 }}
            animate={{ opacity: 1, scale: 1 }}
      ...
      >
        ...
        <Link target='_blank' href="https://drive.google.com/file/d/1AYcLEW5zkwUO99uC_Y2OLtaHGFJp_FOg/view?usp=drive_link" download={true} className='flex items-center gap-2 bg-white dark:bg-white/10 px-7 py-3 border border-black/10 rounded-full group outline-none focus:scale-110 hover:scale-110 active:scale-105 transition cursor-pointer'>
          Download CV <HiDownload className='opacity-60 transition group-hover:translate-y-1' />
        </Link>
        <Link href="https://www.linkedin.com/in/akkid7/" target="_blank" className='flex items-center gap-2 bg-white dark:bg-white/10 p-4 border border-black/10 rounded-full text-gray-700 outline-none focus:scale-[1.15] hover:scale-[1.15] hover:text-gray-950 active:scale-105 dark:text-white/60 transition cursor-pointer'>
          <BsLinkedin />
        </Link>
        <Link href="https://github.com/akkid7" target="_blank" className='flex items-center gap-2 bg-white dark:bg-white/10 p-4 border border-black/10 rounded-full text-[1.35rem] text-gray-700 hover:text-gray-950 dark:text-white/60 transition cursor-pointer outline-none focus:scale-[1.15] hover:scale-[1.15] active:scale-105'>
          <FaGithubSquare />
        </Link>
        <Link href="https://x.com/akkid7" target="_blank" className='flex items-center gap-2 bg-white dark:bg-white/10 p-4 border border-black/10 rounded-full text-[1.35rem] text-gray-700 hover:text-gray-950 dark:text-white/60 transition cursor-pointer outline-none focus:scale-[1.15] hover:scale-[1.15] active:scale-105'>
          <FaTwitter />
        </Link>
      </motion.div>
    </section >
